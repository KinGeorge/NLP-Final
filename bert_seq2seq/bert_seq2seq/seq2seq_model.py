...
class Seq2SeqModel(BasicBert):
    ...
    def generate(self, text, out_max_length=80, beam_size=1, is_poem=False, max_length=256):
    ...
    def sample_generate(self, text, out_max_length=80, top_k=30,
    ...
    def acrostic_generate(self,head):
        token=head
        sentence=head
        for i in range(6):
            token_ids, token_type_ids=self.tokenizer.encode(token)
            token_ids=token_ids[:-1] #去掉结束符
            token_type_ids = token_type_ids[:-1] #去掉结束符
            token_ids = torch.tensor(token_ids, device=self.device).view(1, -1)
            token_type_ids = torch.tensor(token_type_ids, device=self.device).view(1, -1)
            output_logits=self.forward(token_ids,token_type_ids)
            val, idx = torch.topk(output_logits[0][-1], k=3, dim=-1)
            while True:
                char_generated = self.tokenizer.decode(idx[random.randint(0, len(idx) - 1)].unsqueeze(0).cpu().numpy()) # 用模型的tokenizer解码
                if char_generated != token:
                    if char_generated != "，":
                        break
            sentence+=char_generated
            token=char_generated
        return sentence
