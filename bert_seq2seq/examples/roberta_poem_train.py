    ...
    for file1 in files: #遍历文件夹
        if not os.path.isdir(file1): #判断是否是文件夹，不是文件夹才打开
            ...
            for index, row in df.iterrows():
                if type(row[0]) is not str or type(row[4]) is not str:
                    continue
                ...
                encode_text = tokenizer.encode(row[4])[0]
                if word2idx["[UNK]"] in encode_text:
                  # 过滤unk字符
                  continue
                if len(row[4]) == 24 and (row[4][5] == "，" or row[4][5] == "。"):
                    # 五言绝句
                    sents_src.append(row[0] + "##" + "五言绝句")
                    sents_tgt.append(row[4])
                elif len(row[4]) == 32 and (row[4][7] == "，" or row[4][7] == "。"):
                    # 七言绝句
                    sents_src.append(row[0] + "##" + "七言绝句")
                    sents_tgt.append(row[4])
                elif len(row[4]) == 48 and (row[4][5] == "，" or row[4][5] == "。"):
                    # 五言律诗
                    sents_src.append(row[0] + "##" + "五言律诗")
                    sents_tgt.append(row[4])
                elif len(row[4]) == 64 and (row[4][7] == "，" or row[4][7] == "。"):
                    # 七言律诗
